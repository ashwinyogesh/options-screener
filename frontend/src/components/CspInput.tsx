...
const SCORE_LEGEND = [
  ...
  { factor: 'Trend: 52W Dist',  weight: 15,  detail: 'CSP: ≤5%=15 · 5–30% linear 15→0 · >30%=0. CC tent: ≤5%=0 · 5–15%→0→15 · 15–35%→15→0.',
    definition: 'Direction-aware trend factor based on distance from the 52-week (252-trading-day) high. v3.1 rescaled from 25 to 15 pts — 10 pts moved to SMA Alignment + SMA Slope sub-factors.',
    why: 'For CSPs: stocks near their highs have upward momentum and are less likely to gap down through your strike. Far below the 52W high signals a downtrend — puts sold there carry much higher assignment risk. Single smooth linear decay (flat top ≤5%, zero at 30%).',
    formula: 'dist = (Closeₜ − max(Close, 252d)) / max(Close, 252d) × 100\n  pct_below = abs(min(dist, 0))\n  v3.1: 15 pts max, flat ≤5% then linear 15→0 at 30%' },
  { factor: 'Trend: SMA Align', weight: 5,   detail: 'sma_ratio >1.02 = 5 · 1.0–1.02 = 3 · 0.98–1.0 = 1.5 · <0.98 = 0.',
    definition: 'SMA50/SMA200 ratio — captures whether the 50-day average is above the 200-day average (golden cross territory). Restored in v3.1 as an independent 5-pt sub-factor separate from 52W proximity.',
    why: 'SMA alignment is structurally different from 52W proximity: a stock can be near its 52W high yet have a declining SMA50 (distribution), or be far from its 52W high but have a rising SMA50 (early recovery).',
  ...
  { factor: 'Trend: SMA Slope', weight: 5,   detail: '≥0.5%=5 · 0.2–0.5% linear 3→5 · 0–0.2% linear 0→3 · negative=0.',
    ...
    why: 'A rising SMA50 confirms the trend is currently accelerating; a flat or declining SMA50 warns that the structure is deteriorating even if price is still elevated.',
    formula: 'sma50_slope_pct = (SMA50[−1] / SMA50[−11] − 1) × 100\n  10 trading-day window; 0.0 default if insufficient history' },
  { factor: 'RSI(14)',          weight: 20,  detail: 'CSP: 42–62=20 · 35–42 linear 0→20 · 62–75 linear 20→0 · <35 or >75=0.',
    ...
    why: 'Mid-range RSI = healthy trend, neither overheated nor breaking down. Overbought (>75) risks a near-term reversal into your strike; deeply oversold (<35) stocks rarely recover meaningfully within the DTE window.',
    formula: 'Wilder-smoothed RSI(14)\n  v3 cliff fix #2: removed 30–35 floor of 2 pts that created a 4-pt jump at RSI=35\n  Now <35 = 0; 35–42 ramps continuously from 0 to 20\n  v3 rescale: 10 → 20 pts' },
  ...
  { factor: 'Delta',            weight: 25,  detail: 'Smooth bell · |Δ−(−0.225)| ≤ 0.025 = 25 · 0.025–0.075→25→16 · 0.075–0.125→16→9 · 0.125–0.175→9→0 · >0.175=0.',
    ...
    why: 'Delta approximates the probability of expiring in-the-money. −0.225 is the sweet spot for premium vs. risk. v3.1 raised from 20 to 25 pts and replaced step-bands with smooth piecewise-linear decay — eliminates artificial cliff at offset 0.075.',
  ...
  { factor: 'Bid-Ask Spread',  weight: 25,  detail: '≤1%=25 · 1–3%→25→17 · 3–5%→17→9 · 5–8%→9→2 · >8%=0.',
    ...
    why: 'Wide spreads directly erode realized premium. A 10% spread on a $1.00 put loses $0.05–$0.10 on entry alone, and you pay it again on every roll. v3.1 lowered from 30 to 25 pts — rebalanced with Delta which was raised to 25.',
  ...
  { factor: 'Exp Move Buffer', weight: 0,   detail: 'Computed and shown in the table for visibility. Contributes 0 to score in v3.',
    definition: 'How far outside the 0.5× expected move boundary the strike sits, measured in units of the full expected move. Positive = strike is well beyond the reference floor.',
    why: 'Dropped from scoring in v3 (ADR-0007) — the factor was deterministically positive at the configured ideal delta, contributing redundant signal with Δ and %OTM (44%-redundancy stack).',
    formula: 'EM = S × σ × √T    where T = DTE/365\n  EM_half_lower = S − 0.5 × EM\n  sigmas_outside = (EM_half_lower − strike) / EM\n  Returned as em_buffer_pct in the response payload but not scored.' },
  { factor: '% OTM from Spot', weight: 0,   detail: 'Computed and shown in the table for visibility. Contributes 0 to score in v3.',
    definition: 'The raw percentage gap between current stock price and the strike.',
