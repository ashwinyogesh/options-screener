...
const SCORE_LEGEND = [
  ...
  { factor: '52W High Dist.',  weight: 10,  detail: 'CSP curve: ≤5%=10 · 5–10%→10→7.3 · 10–20%→7.3→4.7 · 20–30%→4.7→0 · >30%=0.',
    ...
    formula: 'dist = (Closeₜ − max(Close, 252d)) / max(Close, 252d) × 100\n  pct_below = abs(min(dist, 0))\n  Smooth CSP curve: 5–10% starts at 10.0 (bug fix — was 7.333), decays continuously to 0 at 30%' },
  ...
  { factor: 'Exp Move Buffer', weight: 20,  detail: '≥0.2σ outside=20 · 0–0.2σ→13 · −0.1–0σ→5 · deeper inside=0.',
    definition: 'How far outside the 0.5× expected move boundary the strike sits, measured in units of the full expected move. Positive = strike is well beyond the reference floor; negative = inside it.',
    why: 'Recalibrated to 0.5× EM reference (was 1× EM). A −0.225 delta put naturally sits about 0.25 EM units below the 1× EM lower bound but 0.25 EM above the 0.5× boundary, which is the practical safety threshold for most premium-selling setups.',
    formula: 'EM = S × σ × √T    where T = DTE/365\n  EM_half_lower = S − 0.5 × EM   (reference boundary)\n  sigmas_outside = (EM_half_lower − strike) / EM\n  Positive = strike is below the 0.5σ floor (well protected)' },
  ...
  { factor: 'Annualized ROC',   weight: 10,  detail: '≥20%=10 · 14–20%→7→10 · 8–14%→4→7 · 4–8%→1→4 · <4%=0.',
    ...
    why: 'Recalibrated: full credit now at 20% (was 30%). 20% annualized on secured collateral is an excellent real-world return. The prior 30% threshold only rewarded illiquid chains that simultaneously failed Bid-Ask.',
    formula: 'capital_per_share = strike − credit\n  ROC = (credit / capital_per_share) × (365 / DTE) × 100\n  Full credit threshold lowered from 30% → 20%' },
]
