...
4. Default values for optional config fields (no pre_processors /
   tie_break_key / result_factory) work for CSP/CC-shaped configs.

If Phase 4 needs to evolve the type surface, these tests catch the breaking
change immediately.
"""
...
def test_screener_config_csp_shape_builds_with_minimal_fields():
    ...
    assert cfg.candidate_delta_predicate is None
    assert cfg.tie_break_key is None
    assert cfg.result_factory is None
    assert cfg.strike_sort == "asc"
...
def test_screener_config_ditm_shape_with_hooks():
    """DITM uses pre_processors + tuple tie-break; verify they compose."""

    def _stub_pre(_sym: str, _df: Any, ind: Indicators) -> Indicators:
        return ind  # no-op

    def _ditm_tie_break(s: Any) -> tuple[float, ...]:
        if hasattr(s, "candidate"):
            return (-abs(s.candidate.delta - 0.82),)
        return (0.0,)
    ...
    assert cfg.candidate_delta_predicate is not None
    assert cfg.candidate_delta_predicate(0.65) is True
    assert cfg.candidate_delta_predicate(0.55) is False
    assert cfg.strike_sort == "desc"
